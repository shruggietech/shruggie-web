...
- **Canonical product catalog**: single-sourced product metadata for the
  ...
  `SoftwareSourceCode` metadata, official brand marks, and Glitchpad developer
  discovery
...
- **Blog post CTA**: reusable `PostCTA` (primary → `/contact`, secondary → `/services`) rendered automatically by the shared article template for every current and future post
