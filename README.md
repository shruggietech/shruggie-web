...
| Blog content | Firestore-authoritative Markdown via `next-mdx-remote/rsc`, syntax highlighting via [Shiki](https://shiki.style)         |
...
The production editorial backend architecture is documented
...
Migration, export, cutover, and recovery procedures are in
[`docs/editorial/migration-and-cutover.md`](docs/editorial/migration-and-cutover.md).
...
| `npm run cms:migrate`      | Dry-run or apply the reviewed repository blog migration         |
| `npm run cms:export`       | Export articles and verified asset bytes outside the repository |
...
content/        Recovery blog corpus and authored MDX work case studies
...
- **Blog posts** are created, previewed, and published through the authorized
  `/admin` workspace. The frozen files in `content/blog/` are migration and
  disaster-recovery inputs, not a production authoring path. See
  `ShruggieTech_Website_Specification.md` §7 and the migration runbook.
